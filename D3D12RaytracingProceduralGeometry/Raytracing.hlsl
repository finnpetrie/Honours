...
inline void VisualizePhoton(PhotonPayload payload, float2 screenDims)
{
    // Get the Hit location of the photon
    float3 hitPosition = payload.position.xyz;
    ...
    if(!shadow.hit){
        ...
        screenSpacePhotonMap[0][pixelPos] = float4(1, 1, 1, 1);
...
void Photon_Ray_Gen() {
    screenSpacePhotonMap[0][DispatchRaysIndex().xy] = float4(0, 0, 0, 0);
    ...
    PhotonPayload payload = { float4(0,0,0,0),float4(0,0,0,0),
        0, 0 };
...
void ClosestHit_Photon_Triangle(inout PhotonPayload payload, in BuiltInTriangleIntersectionAttributes attr) {
    //if diffuse - project back to the camera's origin
   // g_renderTarget[DispatchRaysIndex().xy] = float4(0.5, 0, 0, 1);
   ...
   // g_renderTarget[DispatchRaysIndex().xy] = float4(0, 1, 0, 1);
    ...
    float2 uv;
    float2 temp = uv;
    bool splats = inscreenSpace(pos, uv);
   // g_renderTarget[temp.xy] = float4(1, 1, 0, 1);
   ...
   // uv /= screenDims;
    ...
    //else we find the intersection point to the image plane, and check for intersections on the interval.

    //if there weren't, we store the value - a caustic.
}
